...
╰─────────────────────────────────────────────────────────────────────────────╯
	...
	output.WriteString("   ")
	output.WriteString(DescStyle.Render("tnr modify [index|id] --mode prototyping --gpu {t4|a100|h100} --vcpus {4|8|16|32} [--disk-size-gb {100-1000}]"))
	output.WriteString("\n")

	output.WriteString("  ")
	output.WriteString(CommandStyle.Render("Production"))
	output.WriteString("   ")
	output.WriteString(DescStyle.Render("tnr modify [index|id] --mode production --gpu {a100|h100} --num-gpus {1|2|4} [--disk-size-gb {100-1000}]"))
	output.WriteString("\n\n")
	...
	output.WriteString("\n\n")
	output.WriteString("  ")
	output.WriteString(ExampleStyle.Render("# Interactive instance selector and step-by-step wizard"))
	output.WriteString("\n")
	output.WriteString("  ")
	...
	output.WriteString("  ")
	output.WriteString(ExampleStyle.Render("# Interactive mode using instance index"))
	output.WriteString("\n")
	output.WriteString("  ")
	...
	output.WriteString("  ")
	output.WriteString(ExampleStyle.Render("# Upgrade GPU and increase disk (can use instance ID)"))
	output.WriteString("\n")
	output.WriteString("  ")
	output.WriteString(CommandTextStyle.Render("tnr modify abc123 --gpu h100 --disk-size-gb 800"))
	output.WriteString("\n\n")
	...
	output.WriteString("   ")
	output.WriteString(DescStyle.Render("GPU type (prototyping: t4, a100, or h100; production: a100 or h100)"))
	output.WriteString("\n")
